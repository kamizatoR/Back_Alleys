class Admin::SearchesController < ApplicationController
  ...
  def result
    ...
    #   end
    #   #退会していないユーザーのみ取得
    #   @search_results= Kaminari.paginate_array(@uncancelled_user_search_result).page(params[:page]).per(6)
    ...
    if params[:range] == "ユーザー"
      @search_results = EndUser.active.looks(params[:word]).page(params[:page]).per(12)
    else
      @search_results= Post.published.looks(params[:word]).page(params[:page]).per(12)
    end
